...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if (
            ...
            and not options["mycloud"]
            and not options["dropbox"]
        ):
    ...
    def upload_to_mycloud(self, file_name, file_path):
        ...
        r = cloud.create_file(
            file_name, open(file_path, "rb").read(), cloud.BACKUP_DIR_ID
        )
    ...
    def upload_to_dropbox(self, file_name, file_path):
        ...
        try:
            ...
            if (
                ...
                mail_admins(
                    "Postgres backup dropbox save failed!", err.user_message_text
                )
                print(err.user_message_text)
                return
            else:
                mail_admins("Postgres backup dropbox save failed!", err)
                print(err)
                return
