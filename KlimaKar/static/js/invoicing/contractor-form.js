$(function() {
    if ($("#duplicated_phone").length > 0) {
        const contractor_data = JSON.parse(
            $("#duplicated_phone")
                .val()
                .slice(2, -2)
        );
        ...
        let html_content = $("<div>")
            ...
            .append(
                $(
                    '<button style="margin: 0px 4px;" id="swal-force-save" class="btn btn-outline-primary">Zapisz mimo to</button>'
                )
            );
        if (
            $("#id_ignore_duplicated_phone")
                .parents("form")
                .attr("id") === "modal_form"
        ) {
            html_content.append(
                $(
                    '<button style="margin: 0px 4px;" id="swal-set-contractor" class="btn btn-outline-success">Ustaw tego kontrahenta</button>'
                )
            );
        }
        ...
        $(document).on("click", "#swal-set-contractor", function() {
            ...
            let $option = $("<option selected></option>")
                .val(contractor_data["id"].value)
                .text(contractor_data["name"].value);
            $("#id_contractor")
                .append($option)
                .trigger("change");
