$(function() {
    ...
    if ($("#ptu-date-range").length > 0) {
        ...
        $("#ptu-date-range")
            .data("daterangepicker")
            .setEndDate(moment().endOf("isoWeek"));
        $("#ptu-date-range")
            .data("daterangepicker")
            .clickApply();
    }
                ...
                if (invoices_without_commission.length === 0) {
                    ...
                    invoices_without_commission.forEach(function(invoice) {
                        const result_row =
                            ...
                            '</td>\
                            <td><a href="' +
                            invoice.contractor.url +
                            '">' +
                            invoice.contractor.name +
                            "</a></td>\
                            <td>" +
                            invoice.brutto_price +
                            '</td>\
                            <td><a class="btn btn-outline-primary btn-sm btn-table" href="' +
                            invoice.url +
                            '" title="Szczegóły">Szczegóły</a></td>\
                            </tr>';
            ...
            success: function(result) {
                ...
                result.invoices.forEach(function(invoice) {
                    ...
                    var result_row =
                        ...
                        '</td>\
                        <td><button class="btn btn-outline-dark btn-table payed" data-invoice="' +
                        invoice.number +
                        '" data-contractor="' +
                        invoice.contractor.name +
                        '" data-url="' +
