    ...
    function decode_aztec(code) {
        $.ajax({
            ...
            success: function (data) {
                if (data.pk) {
                    $("#id_aztec").addClass('is-invalid');
                    if ($("#id_aztec").parent().find('div.invalid-feedback').length == 0) {
                        $("#id_aztec").parent().append('<div class="invalid-feedback">Ten pojazd jest już zapisany.</div>')
                    }
                } else {
                    $("#id_contractor").removeClass('is-invalid');
                    $("#id_aztec").parent().find('div.invalid-feedback').remove();
