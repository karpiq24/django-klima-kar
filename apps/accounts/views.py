...
class FirstStepLoginView(View):
    ...
    def _authenticate(self, request, username, password):
        ...
        defender.utils.add_login_attempt_to_db(
            request, login_valid=True if user else False, username=username
        )

        user_not_blocked = defender.utils.check_request(
            request, login_unsuccessful=False if user else True, username=username
        )
    ...
    def _account_locked_response(self):
        ...
        return JsonResponse(
            {"status": "error", "message": message, "blocked": True}, status=401
        )
