...
class UserSession(models.Model):
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name="Użytkownik"
    )
    session = models.ForeignKey(Session, on_delete=models.CASCADE, verbose_name="Sesja")
    client_ip = models.CharField(
        max_length=64, blank=True, null=True, verbose_name="Adres IP"
    )
    country = models.CharField(
        max_length=64, blank=True, null=True, verbose_name="Kraj"
    )
    user_agent = models.TextField(blank=True, null=True, verbose_name="User agent")
    created = models.DateTimeField(auto_now_add=True, verbose_name="Czas zalogowania")
    ...
    def __str__(self):
        return "{}: {} ({})".format(self.user, self.client_ip, self.created.date())
