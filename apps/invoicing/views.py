...
from apps.invoicing.forms import (
    SaleInvoiceModelForm,
    ContractorModelForm,
    SaleInvoiceItemsInline,
    ServiceTemplateModelForm,
    EmailForm,
    RefrigerantWeightsInline,
    CorrectiveSaleInvoiceModelForm,
)
from apps.invoicing.tables import (
    SaleInvoiceTable,
    ContractorTable,
...
from apps.invoicing.filters import (
    SaleInvoiceFilter,
    ContractorFilter,
    ServiceTemplateFilter,
)
from apps.invoicing.functions import (
    get_next_invoice_number,
    generate_refrigerant_weights_report,
)
...
class SaleInvoiceDetailView(SingleTableAjaxMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["back_url"] = (
            reverse("invoicing:sale_invoices")
            + "?"
...
class SaleInvoiceCreateView(CreateWithInlinesView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if not self.invoice_type or self.invoice_type not in dict(
            SaleInvoice.INVOICE_TYPES
        ):
            raise Http404()
...
class SaleInvoiceUpdateView(UpdateWithInlinesView):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if (
            self.model == SaleInvoice
            and self.object.invoice_type == SaleInvoice.TYPE_CORRECTIVE
        ):
...
class CorrectiveSaleInvoiceUpdateView(SaleInvoiceUpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["title"] = context["title"].replace(
            ")", " do faktury {})".format(self.invoice.original_invoice)
        )
...
class ServiceTemplateDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        key = "{}_params".format(self.model.__name__)
        context["back_url"] = (
            reverse("invoicing:service_templates")
            + "?"
            + urlencode(self.request.session.get(key, ""))
        )
        return context
...
class ServiceTemplateCreateView(CreateView):
    ...
    def form_valid(self, form):
        messages.add_message(
            self.request,
            messages.SUCCESS,
            '<a href="{}">Dodaj kolejną usługę.</a>'.format(
...
class ContractorDetailView(MultiTableAjaxMixin, DetailView):
    ...
    def get_tables_data(self):
        return {
            self.SALE_INVOIDE_ID: SaleInvoice.objects.filter(
...
class ContractorCreateView(CreateView):
    ...
    def form_valid(self, form):
        messages.add_message(
            self.request,
            messages.SUCCESS,
            '<a href="{}">Dodaj kolejnego kontrahenta.</a>'.format(
    ...
    def get_queryset(self):
        ...
        if self.q:
            ...
            qs = qs.filter(
                ...
                | Q(phone_1__icontains=self.q)
                | Q(phone_2__icontains=self.q)
            )
        return qs
...
class ContractorGUS(View):
    def get(self, request, *args, **kwargs):
        ...
        if request_type == "address":
            ...
            response_data = {
                "pkd": sorted(
                    GUS.get_gus_pkd(nip), key=lambda k: k["main"], reverse=True
                )
            }
