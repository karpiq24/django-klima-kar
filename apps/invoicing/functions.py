...
def get_next_invoice_number(invoice_type):
    ...
    if invoice_type == SaleInvoice.TYPE_VAT:
        ...
        invoices = (invoices | SaleInvoice.objects.filter(
            invoice_type=SaleInvoice.TYPE_VAT, number_year=year)).distinct()
    elif invoice_type == SaleInvoice.TYPE_PRO_FORMA:
        invoices = (invoices | SaleInvoice.objects.filter(
            invoice_type=SaleInvoice.TYPE_WDT_PRO_FORMA, number_year=year)).distinct()
    elif invoice_type == SaleInvoice.TYPE_WDT_PRO_FORMA:
        invoices = (invoices | SaleInvoice.objects.filter(
            invoice_type=SaleInvoice.TYPE_PRO_FORMA, number_year=year)).distinct()
