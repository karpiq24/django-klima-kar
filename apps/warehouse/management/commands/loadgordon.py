...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with requests.Session() as s:
            ...
            for row in soup.find_all("tr", {"class": "clickable"}):
                number = (
                    row.find("td", {"class": "doc-item-doc-id-col"})
                    .find("a")
                    .text.strip()
                )
                date = row.find(
                    "td", {"class": "doc-item-date-create-col"}
                ).text.strip()
                value_netto = float(
                    row.find("td", {"class": "doc-item-net-col"})
                    .text.strip()
                    .split(" ")[0]
                    .replace(",", ".")
                )
                ...
                if (
                    ...
                    and not Invoice.objects.filter(
                        number=number, supplier=self.settings.GORDON_SUPPLIER
                    ).exists()
                ):
    ...
    def parse_invoice(self, html_content, number, issue_date, value_netto):
        ...
        new_wares = 0
        for row in soup.find_all("tr", {"class": "dgriditem"}) + soup.find_all(
            "tr", {"class": "dgridaltitem"}
        ):
            ...
            try:
                ...
                index2 = "{} {}".format(
                    index_split[-1], " ".join(index_split[:-1])
                ).strip()
                index3 = " ".join(index_split[:-1])
                ware = Ware.objects.get(
                    Q(index=index)
                    | Q(index_slug=Ware.slugify(index))
                    | Q(index=index2)
                    | Q(index_slug=Ware.slugify(index2))
                    | Q(index=index3)
                    | Q(index_slug=Ware.slugify(index3))
                )
