...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with requests.Session() as s:
            ...
            data = {
                ...
                "access_token": soup.find("input", attrs={"name": "access_token"})[
                    "value"
                ],
                ...
                for row in soup.find("tbody").find_all("tr"):
                    ...
                    if (
                        ...
                        and not Invoice.objects.filter(
                            number=number, supplier=self.settings.PROFIAUTO_SUPPLIER
                        ).exists()
                    ):
    ...
    def parse_invoice(self, xml_string):
        ...
        invoice = Invoice.objects.create(
            number=number, date=issue_date, supplier=self.settings.PROFIAUTO_SUPPLIER
        )
