...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        month_dates = list(
            rrule.rrule(rrule.MONTHLY, bymonthday=1, dtstart=date_from, until=date_to)
        )
        with requests.Session() as s:
            ...
            data = {
                "_csrf_token": soup.find("input", attrs={"name": "_csrf_token"})[
                    "value"
                ],
                ...
                for invoice in r.json().get("data", []):
                    ...
                    if (
                        ...
                        and not Invoice.objects.filter(
                            number=number, supplier=self.settings.ZATOKA_SUPPLIER
                        ).exists()
                    ):
    ...
    def parse_invoice(self, html_content, number, issue_date, value_netto):
        ...
        invoice = Invoice.objects.create(
            number=number, date=issue_date, supplier=self.settings.ZATOKA_SUPPLIER
        )
        ...
        for row in soup.find_all("tr", {"class": "mod-list-item"}):
            ...
            price = float(
                cells[5].find(text=True, recursive=False).strip().replace(",", ".")
            )
