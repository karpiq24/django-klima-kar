...
class WareFilter(django_filters.FilterSet):
    IN_STOCK = "in"
    NOT_IN_STOCK = "not_in"
    STOCK_CHOICES = ((IN_STOCK, "Na stanie"), (NOT_IN_STOCK, "Brak na stanie"))
    ...
    stock = django_filters.ChoiceFilter(
        choices=STOCK_CHOICES, method="stock_filter", widget=forms.Select()
    )
    ...
    def index_filter(self, queryset, name, value):
        return queryset.filter(
            Q(index__icontains=value) | Q(index_slug__icontains=Ware.slugify(value))
        )
    ...
    def purchase_date_filter(self, queryset, name, value):
        ...
        return queryset.filter(
            invoiceitem__invoice__date__gte=date_from,
            invoiceitem__invoice__date__lte=date_to,
        ).distinct()
