...
def generate_ware_inventory(queryset):
    ...
    bold_money = workbook.add_format(
        {'num_format': '#,##0.00 zł', 'bold': True})
    border = workbook.add_format({'border': 1})
    bold_border = workbook.add_format({'border': 1, 'bold': True})
    border_money = workbook.add_format(
        {'border': 1, 'num_format': '#,##0.00 zł'})
    columns = [
        ('Lp.', 5),
        ('Nr kat.', 35),
        ('Nazwa', 35),
    ...
    worksheet.write(0, 1, "INWENTARYZACJA NA DZIEŃ {}".format(
    ...
    for ware in queryset.order_by('index'):
        ...
        if ware.last_price:
            worksheet.write(row, col + 5, ware.stock *
                            ware.last_price, border_money)
        else:
    ...
    worksheet.write(
        row + 3, 1, 'Remanent zakończono na pozycji {}.'.format(row - 3))
