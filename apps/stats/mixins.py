...
class ChartDataMixin(object):
    def get_response_data_template(
        self,
        chart_type="line",
        legend_display=True,
        values_prefix="",
        ...
        response_data = {
            ...
            "options": {"legend": {"display": legend_display}},
            "custom": {
...
class BigChartHistoryMixin(ChartDataMixin, View):
    ...
    def _annotate_sum(self, qs):
        return qs.annotate(
            total=Sum(
                F(self.price_field) * F(self.quantity_field), output_field=FloatField()
            )
        )
    ...
    def _get_date_filter_kwargs(self, date):
        return {"{}__gte".format(self.date_field): date}
    ...
    def _get_week(self):
        ...
        self.objects = self.objects.values_list("total", self.date_field).order_by(
            self.date_field
        )
        ...
        self.response_data["data"]["labels"] = [
            DAYS[(date + relativedelta(days=i)).weekday()]
            for i in range(days_between + 1)
        ]
    ...
    def _get_month(self):
        ...
        self.response_data["data"]["labels"] = [
            (date + relativedelta(days=i)).strftime("%d/%m")
            for i in range(days_between + 1)
        ]
        self.response_data["data"]["datasets"].append(
            self.get_dataset(values, COLORS[0])
    ...
    def _get_year(self):
        ...
        self.objects = self.objects.annotate(
            month=ExtractMonth(self.date_field), year=ExtractYear(self.date_field)
        ).values("year", "month")
        self.objects = self._annotate(self.objects)
        self.objects = self.objects.values_list("year", "month", "total").order_by(
            "year", "month"
        )
        values = list(self.objects.values_list("total", flat=True))
    ...
    def _get_all_monthly(self):
        years = (
            self.objects.annotate(year=ExtractYear(self.date_field))
        ...
        for i, year in enumerate(years):
            ...
            year_objects = year_objects.annotate(
                month=ExtractMonth(self.date_field)
            ).values("month")
    ...
    def _get_all_yearly(self):
        self.objects = self.objects.annotate(year=ExtractYear(self.date_field)).values(
            "year"
        )
        self.objects = self._annotate(self.objects)
        self.objects = (
            self.objects.values_list("year", "total").exclude(total=0).order_by("year")
        )
        ...
        self.response_data["data"]["datasets"].append(
            self.get_dataset(
                list(self.objects.values_list("total", flat=True)), COLORS[0]
            )
        )
